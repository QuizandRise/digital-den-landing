...
assert.equal(ROLE_CAPABILITIES.team_member.viewProjectAudit, false, "Team Member must not view project audit information");
assert.equal(ROLE_CAPABILITIES.team_member.assignInternalResources, false, "Team Member must not assign or reassign work");
assert.equal(ROLE_CAPABILITIES.team_member.setClientPrice, false, "Team Member must not change client price");
assert.equal(ROLE_CAPABILITIES.client.viewInternalCompensation, false, "Client must not see internal compensation");
assert.equal(ROLE_CAPABILITIES.client.viewInternalNotes, false, "Client must not see internal notes");
